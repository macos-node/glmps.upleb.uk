...
// Monochrome ramp from a themed CSS var (coral on upleb / emerald on fizx for
// --primary; amber / purple for --accent). Rank 0 = full strength, fading down
// the tail. Palette-agnostic — colours come from the CSS vars, so this file
// stays byte-identical across the upleb/fizx pair while reading correctly on
// each. Genre keeps its own semantic `--c-g-*` colours.
function ramp(varName: string, rank: number, total: number): string {
  const alpha = total <= 1 ? 1 : 0.4 + 0.6 * (1 - rank / (total - 1));
  return `hsl(var(${varName}) / ${alpha.toFixed(3)})`;
}
 ...
 * proportional stacked bar plus a counted legend. Shared design across the
 * web viewer and the mobile app; all data is tallied client-side from the
 * already-loaded releases (no extra fetch). Genre uses any-slot tallying
 * (a release with N slugs contributes N tallies), matching GenreBar.
 */
export default function StatsBreakdown({ releases }: { releases: Release[] }) {
  ...
  const decade = useMemo<Row[]>(() => {
    ...
    return sorted.map(([d, count], i) => ({
      ...
      label: d,
      count,
      color: ramp("--primary", i, sorted.length),
    }));
  }, [releases]);
  ...
  const format = useMemo<Row[]>(() => {
    ...
    return ordered.map((f, i) => ({
      ...
      label: f,
      count: m.get(f) ?? 0,
      color: ramp("--accent", i, ordered.length),
    }));
  }, [releases]);
